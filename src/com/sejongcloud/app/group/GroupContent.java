...
import com.sejongcloud.app.main.Main;
...
public class GroupContent extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		JSONArray json = JSONfunctions
				.getJSONfromURL("http://yhjun0229.cafe24.com/sejongstick/php/memberList.php");
		try {
		...
		if (follower_count == 0) {
			((TextView) findViewById(R.id.followerCount))
					.setText("아무도 참여하지 않았습니다.");
			((ProgressBar) findViewById(R.id.followerCountBar))
					.setVisibility(View.GONE);
		} else if (follower_count == count) {
			((ProgressBar) findViewById(R.id.followerCountBar)).setMax(count);
			((ProgressBar) findViewById(R.id.followerCountBar))
					.setProgress(follower_count);
			((TextView) findViewById(R.id.followerCount))
					.setText("남은 자리가 없습니다");
			...
			((ProgressBar) findViewById(R.id.followerCountBar))
					.setProgress(follower_count);
			((TextView) findViewById(R.id.followerCount))
					.setText(follower_count + "명이 참여중 입니다.");
		}
	}
	...
	public void profileSetting() {
		linear = (LinearLayout) View.inflate(GroupContent.this,
				R.layout.group_follow, null);
	...
	public void followThread() {
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				try {
					URL url = new URL(
							"http://yhjun0229.cafe24.com/sejongstick/php/follow.php");
					...
					sb.append("follower_name").append("=").append(name)
							.append("&");
					sb.append("follower_email").append("=").append(email)
							.append("&");
					sb.append("follower_hp").append("=").append(hp).append("&");
					sb.append("follower_password").append("=").append(password)
							.append("");
	...
	private Handler followHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			Toast.makeText(GroupContent.this, "참여하셨습니다", Toast.LENGTH_LONG)
					.show();
			Intent i = new Intent(GroupContent.this, Main.class)
