...
public class Tab2 extends ListActivity {
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				try {
					...
					noticeAdapter = new NoticeAdapter(Tab2.this,
							R.layout.tab2_inner, result);
					// 시간걸리는 처리
	...
	Handler mAfter = new Handler() {
		public void handleMessage(Message msg) { // UI
			dialog.dismiss();
			noticeAdapter = new NoticeAdapter(Tab2.this, R.layout.tab2_inner,
					result);
			setListAdapter(noticeAdapter);
		}
	};
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if ((keyCode == KeyEvent.KEYCODE_BACK)) {
			...
			if (backKeyClick == 1) {
				...
				Toast t = Toast.makeText(Tab2.this, "'뒤로' 버튼을  한번 더 누르면 종료됩니다",
						Toast.LENGTH_SHORT);
